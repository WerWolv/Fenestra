/**
 * This is a simple forwarder that launches the main application executable with the
 * same command line as the current process. The reason for this is that even
 * though the main executable is a GUI application in general, it also has a command line
 ...
 * around in the background if we launch the application from the explorer. This forwarder
 * will get rid of the console window if the application was launched from the explorer and
 * enables ANSI escape sequences if the application was launched from the command line.
 *
...
void setupConsoleWindow() {
    ...
    // Check if the executable was launched from the explorer or from the command line
    if (::GetCurrentProcessId() == processId) {
        ...
        // Set the __FENESTRA_FORWARD_CONSOLE__ environment variable,
        // to let the application know that it was launched from the forwarder
        // and that it should forward it's console output to us
        ::SetEnvironmentVariableA("__FENESTRA_FORWARD_CONSOLE__", "1");
    }
}
...
int launchExecutable() {
    ...
    // Create a new process for fenestra-gui.exe with the same command line as the current process
    if (::CreateProcessW(executableFullPath.wstring().c_str(), ::GetCommandLineW(), nullptr, nullptr, FALSE, 0, nullptr, nullptr, &startupInfo, &process) == FALSE) {
        ...
        auto errorMessage = fmt::format("Failed to start Application:\n\nError code: 0x{:08X}\n\n{}", errorCode, errorMessageString);

        // Display a message box with the error
        ::MessageBoxA(nullptr, errorMessage.c_str(), "Application Forwarder", MB_OK | MB_ICONERROR);
