...
namespace fene::subcommands {
    ...
    void processArguments(const std::vector<std::string> &args) {
        ...
        // (important because this function will exit the application if an instance is already opened,
    ...
    void forwardSubCommand(const std::string &cmdName, const std::vector<std::string> &args) {
        ...
        RequestSendMessageToMainInstance::post(fmt::format("command/{}", cmdName), data);
    }
