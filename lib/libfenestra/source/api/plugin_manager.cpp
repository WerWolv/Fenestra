...
namespace fene {
    ...
    static uintptr_t loadLibrary(const std::fs::path &path) {
            ...
            if (handle == uintptr_t(INVALID_HANDLE_VALUE) || handle == 0) {
                log::error("Loading library '{}' failed: {} {}!", wolv::util::toUTF8String(path.filename()), ::GetLastError(), fene::formatSystemError(::GetLastError()));
                return 0;
            }
    ...
    static void unloadLibrary(uintptr_t handle, const std::fs::path &path) {
            ...
            if (handle != 0) {
                if (FreeLibrary(HMODULE(handle)) == FALSE) {
                    log::error("Error when unloading library '{}': {}!", wolv::util::toUTF8String(path.filename()), fene::formatSystemError(::GetLastError()));
                }
            }
        #else
            if (handle != 0) {
    ...
    Plugin::Plugin(const std::string &name, const PluginFunctions &functions) {
        m_handle        = 0;
    ...
    bool Plugin::initializePlugin(const std::string &requiredVersion) const {
        ...
        if (!requiredVersion.starts_with(requestedVersion)) {
            if (requestedVersion.empty()) {
                log::warn("Plugin '{}' did not specify a compatible version, assuming it is compatible with the current application version.", wolv::util::toUTF8String(m_path.filename()));
            } else {
                log::error("Refused to load plugin '{}' which was built for a different application version: '{}'", wolv::util::toUTF8String(m_path.filename()), requestedVersion);
                return false;
            }
        }
    ...
    void PluginManager::initializeNewPlugins(const std::string &requiredVersion) {
        for (const auto &plugin : getPlugins()) {
            if (!plugin.isLoaded())
                std::ignore = plugin.initializePlugin(requiredVersion);
        }
    }
    ...
    void PluginManager::addPlugin(const std::string &name, PluginFunctions functions) {
