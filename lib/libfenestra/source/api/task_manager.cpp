...
struct std::hash<SourceLocationWrapper> {
    std::size_t operator()(const SourceLocationWrapper& s) const noexcept {
        ...
        auto h3 = std::hash<fene::u32>{}(s.location.column());
        auto h4 = std::hash<fene::u32>{}(s.location.line());
...
namespace fene {
    ...
    void TaskManager::doLater(std::function<void()> function) {
        std::scoped_lock lock(s_deferredCallsMutex);

        s_deferredCalls.emplace_back(std::move(function));
    }

    void TaskManager::doLaterOnce(std::function<void()> function, std::source_location location) {
        std::scoped_lock lock(s_deferredCallsMutex);
    ...
    void TaskManager::runWhenTasksFinished(std::function<void()> function) {
    ...
    void TaskManager::setCurrentThreadName(const std::string &name) {
        ...
        std::ranges::copy(name | std::views::take(255), s_currentThreadName.begin());
            ...
            if (setThreadDescription != nullptr) {
                const auto longName = fene::utf8ToUtf16(name);
