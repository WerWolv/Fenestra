    ...
    namespace impl {
        ...
        void redirectToFile() {
            ...
            for (const auto &path : paths::Logs.all()) {
                ...
                s_loggerFile = wolv::io::File(path / fmt::format("{0:%Y%m%d_%H%M%S}.log", fmt::localtime(std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()))), wolv::io::File::Mode::Create);
        ...
        void addLogEntry(std::string_view project, std::string_view level, std::string_view message) {
            getLogEntries().push_back(LogEntry { project.data(), level.data(), message.data() });
        }
