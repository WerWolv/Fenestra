    ...
    void registerThemeHandlers() {
        RequestInitThemeHandlers::subscribe([] {
...

            {
                ...
                ThemeManager::addThemeHandler("fenestra", ImHexColorMap,
                    [](u32 colorId) -> ImColor {
                        return static_cast<ImGuiExt::ImGuiCustomData *>(GImGui->IO.UserData)->Colors[colorId];

                    },
                    [](u32 colorId, ImColor color) {
                        static_cast<ImGuiExt::ImGuiCustomData *>(GImGui->IO.UserData)->Colors[colorId] = color;
                    }
                );
            }
